...
handled by the companion `wxkey` CLI and may request administrator privileges to
read the local WeChat process memory. The default no-SIP path uses ad-hoc signing
of the local WeChat.app when macOS denies `task_for_pid`.
